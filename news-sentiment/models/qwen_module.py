...
class QwenPTuningModule(pl.LightningModule):
    def __init__(self, cfg, train_data, val_data):
        ...
        base_model = AutoModelForSequenceClassification.from_pretrained(cfg.model_name_or_path, num_labels=cfg.num_labels)
        base_model.config.pad_token_id = self.tokenizer.pad_token_id
        self.model = get_peft_model(base_model, LoraConfig(task_type=TaskType.SEQ_CLS))
    ...
    def tokenize_batch(self, batch):
        return self.tokenizer(batch["text"], padding=True, truncation=True, max_length=self.cfg.max_length)
    ...
    def prepare_dataloader(self, data, shuffle=False):
        ...
        return DataLoader(tokenized, batch_size=self.cfg.batch_size, shuffle=shuffle)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/auc", roc_auc_score(labels, output.logits.softmax(dim=-1)[:, 1].detach().cpu()), prog_bar=True)
